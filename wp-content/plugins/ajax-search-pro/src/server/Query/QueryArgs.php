...
class QueryArgs {
	...
	public static function get( int $search_id, array $o = array(), array $args = array() ): array {
		...
		if ( $sd['groupby_cpt_title'] ) {
			if ( !isset($args['cpt_query']) ) {
				$args['cpt_query'] = $default_query_arr;
			} else {
				$args['cpt_query']['groupby'] = 'title';
			}
		}
		if ( $sd['groupby_term_title'] ) {
			if ( !isset($args['term_query']) ) {
				$args['term_query'] = $default_query_arr;
			} else {
				$args['term_query']['groupby'] = 'title';
			}
		}
		if ( $sd['groupby_user_title'] ) {
			if ( !isset($args['user_query']) ) {
				$args['user_query'] = $default_query_arr;
			} else {
				$args['user_query']['groupby'] = 'title';
			}
		}
		if ( $sd['groupby_attachment_title'] ) {
			if ( !isset($args['attachment_query']) ) {
				$args['attachment_query'] = $default_query_arr;
			} else {
