    ...
    class WD_ASP_Priorities {
        ...
        static function ajax_get_posts() {
            ...
            if (isset($o['p_asp_filter']) && $o['p_asp_filter'] != '') {
                $w_filter = "AND $wpdb->posts.post_title LIKE '%" . ASP_Helpers::escape($o['p_asp_filter']) . "%'";
            }

            if (isset($o['p_asp_post_type']) && $o['p_asp_post_type'] != 'all') {
                $w_post_type = "AND $wpdb->posts.post_type = '" . ASP_Helpers::escape($o['p_asp_post_type']) . "'";
            }
        ...
        static function ajax_set_priorities() {
            ...
            $priority_rows   = (int) $wpdb->get_var( "SELECT 1 FROM ".wd_asp()->db->table("priorities")." LIMIT 1" );
            $count = $wpdb->num_rows;
            update_site_option('_asp_priority_count', $count);
