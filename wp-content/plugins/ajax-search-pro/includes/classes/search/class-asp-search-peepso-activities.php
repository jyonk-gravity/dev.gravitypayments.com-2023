...
if (!class_exists('ASP_Search_PeepsoActivites')) {
    /**
     * Comment search class
     *
     * @class       ASP_Search_PeepsoActivites
     * @version     1.0
     * @package     AjaxSearchPro/Abstracts
     * @category    Class
     * @author      Ernest Marcinko
     */
    class ASP_Search_PeepsoActivites extends ASP_Search_CPT {
        /**
            ...
            if (is_array($r)) {
                foreach ($r as $k => $v) {
                    ...
                    if ( $v->post_type == 'peepso-post' ) {
                        $r[$k]->link = PeepSo::get_page('activity_status') . $v->title . '/';
                    } else {
                        ...
                        $r[$k]->link = PeepSo::get_page('activity_status') . $parent_post->post_title . '/#comment.'.$parent_activity->act_id . '.' . $v->parent_activity_id . '.' . $v->activity_id . '.' . $v->id;
                    }
                    ...
                    if ( ASP_mb::strlen($v->content) > ASP_mb::strlen($v->title) )
                        $v->title .= '..';
