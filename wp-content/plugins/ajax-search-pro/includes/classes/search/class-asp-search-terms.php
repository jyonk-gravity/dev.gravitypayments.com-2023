                ...
                $wpml_query = "
    				EXISTS (
    					...
    					WHERE
    	                    ...
    	                    wpml.language_code LIKE '" . ASP_Helpers::escape($args['_wpml_lang']) . "' AND
        ...
        protected function post_process() {
            ...
            if ( $image_settings['show_images'] != 0 ) {
                foreach($term_res as $k => $result) {
                    ...
                    if ( !empty($image) ) {
                        ...
                        if ( $image_settings['image_cropping'] == 0 ) {
                            ...
                            if ( strpos( $image, "mshots/v1" ) === false ) {
                                ...
                                $term_res[$k]->image = bfi_thumb( $image, $bfi_params );
                            } else {
                                $term_res[$k]->image = $image;
                            }
                        }
                    ...
                    if ( empty($term_res[$k]->image) && !empty($sd['tax_image_default']) ) {
                        $term_res[$k]->image = $sd['tax_image_default'];
                    }

                    if ( !empty($term_res[$k]->image) ) {
                        $term_res[$k]->image = ASP_Helpers::fixSSLURLs($term_res[$k]->image);
                    }
                }
            }
            ...
            foreach ($term_res as $k=>$v) {
                ...
                $_descriptionlength = isset($sd['tax_res_descriptionlength']) ? $sd['tax_res_descriptionlength'] : 220;
