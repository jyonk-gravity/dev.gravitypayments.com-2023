                        ...
                        if ( !$user_meta && function_exists('get_field_object') ) {
                            ...
                            if ( count($acf_field_o) > 0 ) {
                                ...
                                foreach ( $acf_field_o as $acf_f_o_k => $acf_f_o_v) {
                                    ...
                                    $add_arr[] = array(
                                        'label' => $acf_f_o_v . (strpos(trim($val), 'checked') !== false && $cfk == 0 ? '**' : ''),
                                        'value' => $acf_f_o_k
                                    );
                                    ++$cfk;
                                }
                            ...
                            if (count($acf_field_o) > 0) {
                                ...
                                foreach ($acf_field_o as $acf_f_o_k => $acf_f_o_v) {
                                    ...
                                    if ( $filter->data['multiple'] ) {
                                        $add_arr[] = array(
                                            'label' => $acf_f_o_v . (strpos(trim($val), 'checked') !== false ? '**' : ''),
                                            'value' => $acf_f_o_k
                                        );
                                    } else {
                                        $add_arr[] = array(
                        ...
                        if (!$_used_acf) {
                            ...
                            if (!is_wp_error($cf_results) && count($cf_results) > 0) {
                                foreach ($cf_results as $cfk => $cf_result) {
                                    ...
                                    if ( $filter->data['multiple'] ) {
                                        $add_arr[] = array(
                                            'label' => $cf_result->meta_value . (strpos(trim($val), 'checked') !== false ? '**' : ''),
                                            'value' => $cf_result->meta_value
                                        );
                                    } else {
                                        $add_arr[] = array(
                ...
                foreach ($lines as $kk => $val) {
                    ...
                    if ( strpos(trim($val), '{get_values') === 0 ) {
                        ...
                        if ( !$user_meta && function_exists('get_field_object') ) {
                            ...
                            if (count($acf_field_o) > 0) {
                                foreach ($acf_field_o as $acf_f_o_k => $acf_f_o_v) {
                                    $acf_f_o_v = asp_icl_t('[CF Filter] (' .$id. ') ' . $acf_f_o_v, $acf_f_o_v);
                                    $add_arr[] = array(
                                        'label' => $acf_f_o_v . (strpos(trim($val), 'checked') !== false ? '**' : ''),
                                        'value' => $acf_f_o_k
                                    );
