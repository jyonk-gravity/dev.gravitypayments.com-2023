...
if (!class_exists("wpdreamsBlogselect")) {
    ...
    class wpdreamsBlogselect extends wpdreamsType {
        ...
        function getType() {
            parent::getType();
            $this->processData();
            $this->blogs = get_blogs_of_user( get_current_user_id() );
            echo "
          ...
          <legend>" . $this->label . "</legend>";
            ...
            if ( !is_wp_error($this->blogs) ) {
                foreach ($this->blogs as $k => $blog) {
                    if ( $this->selected == null || !in_array($blog->userblog_id, $this->selected) ) {
                        echo '<li class="ui-state-default" bid="' . $blog->userblog_id . '">' . $blog->blogname . '</li>';
                    }
                }
            }
            echo "</ul></div>";
