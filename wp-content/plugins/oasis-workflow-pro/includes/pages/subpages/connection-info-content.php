            ...
            <tr>
                ...
                <td>
                    <select id="step-status-select" name="step-status-select">
						...
						$block_post_statuses = apply_filters( 'owf_block_post_statuses_on_conn_settings',
							$block_post_statuses );
						...
						foreach ( $status_array as $status_slug => $status_object ) {
							...
							echo "<option value='" . esc_attr( $status_slug ) . "'>" .
					...
					foreach ( $oasiswf_path as $k => $v ) {
						$str = '<td>
                         <label for="path-opt-' . esc_attr( $v[1] ) . '">
									<input type="radio" id="path-opt-' . esc_attr( $v[1] ) .
						       '" name="path-opt" value="' . esc_attr( $v[1] ) . '" ' . '/> ' . esc_html( $v[0] ) . '
                              </label>
								</td>';
