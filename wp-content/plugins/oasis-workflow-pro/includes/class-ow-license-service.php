...
class OW_License_Service {
	...
	public function activate_license( $license_key, $license_status_option_name, $product_name ) {
		...
		if ( ! check_admin_referer( 'owf_license_nonce', 'owf_license_nonce' ) ) {
			return;
		} // get out if we didn't click the Save button
		...
		// Construct option name for license expiration as per each addon
		$license_expiry_option_name = str_replace( "status", "expiry", $license_status_option_name );
		...
		// Call the custom API.
		// phpcs:ignore WordPressVIPMinimum.Performance.RemoteRequestTimeout
		...
		// Save license expiration
		update_option( $license_expiry_option_name, $license_data->expires );
