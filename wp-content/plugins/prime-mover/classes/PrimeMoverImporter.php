...
class PrimeMoverImporter implements PrimeMoverImport
{
    ...
    public function computeExtractVariables($ret = [])
    {
        $file_path = $ret['file'];
        $extracting_tar = false;
        if ($this->getSystemFunctions()->hasTarExtension($file_path)) {
            $extracting_tar = true;
        }
    ...
    public function dropCustomTables($ret = [], $blogid_to_import = 0, $files_array = [], $start_time = 0)
    {
        ...
        if (isset($ret['tables_to_drop'])) {
            ...
            $alltables = $this->getSystemFunctions()->getTablesforReplacement($blogid_to_import);
    ...
    protected function maybeRestoreDefaultUserRole($blog_id = 0)
    {
        ...
        if (!$this->getSystemFunctions()->getOption($current_role_option)) {
            ...
            do_action('prime_mover_log_processed_events', 'Imported options does not have user role set. Emergency restoration of default user role to avoid breaking restore process.', $blog_id, 'import', __FUNCTION__, $this);
        }
    }
    ...
    public function renameDbPrefix($ret = [], $blogid_to_import = 0, $files_array = [], $start_time = 0)
    {
        ...
        foreach ($package_exported_tables as $table_to_validate) {
            $table_to_validate = filter_var($table_to_validate, FILTER_SANITIZE_STRING);
    ...
    {        
        ...
        if ($doing_count_table_rows_retry) {            
            ...
            $all_tables = $this->getSystemFunctions()->getTablesforReplacement($blogid_to_import);
