...
{    
    ...
    public function primeMoverValidateFootprintData($footprint = [])
    {
        ...
        $error_string = '';
        $correct_keys = apply_filters('prime_mover_footprint_keys', ['plugins', 'stylesheet', 'template', 'using_child_theme', 'site_url', 'wp_root', 'db_prefix'], $footprint);
        ...
        $template_name = '';
        $template_version = '';
        if ((isset($footprint_temp['template'])) && (is_array($footprint_temp['template']))) {
            $count_template = count($footprint_temp['template']);
            if (1 === $count_template) {
                $template_name = key($footprint_temp['template']);
                $template_version = reset($footprint_temp['template']);
                if ((!empty($template_name)) && (!empty($template_version))) {
                    $template_valid	= true;
                }
            }
        }
        if ( ! $template_valid ) {
            if ( ! $template_name ) {
                $errors[] = esc_html__('Error: Missing theme template name.', 'prime-mover');
            }
            if ( ! $template_version ) {
                $errors[] = esc_html__('Error: Missing theme version number. Please add.', 'prime-mover');
            }
        }
        $stylesheet_valid = false;
        $stylesheet_name = '';
        $stylesheet_version = '';
        if ((isset($footprint_temp['stylesheet'])) && (is_array($footprint_temp['stylesheet']))) {
            $count_stylesheet = count($footprint_temp['stylesheet']);
            if (1 === $count_stylesheet) {
                $stylesheet_name = key($footprint_temp['stylesheet']);
                $stylesheet_version = reset($footprint_temp['stylesheet']);
                if ((!empty($stylesheet_name)) && (!empty($stylesheet_version))) {
                    $stylesheet_valid = true;
                }
            }
        }
        if ( ! $stylesheet_valid ) {
            if ( ! $stylesheet_name ) {
                $errors[] = esc_html__('Error: Missing active theme template name.', 'prime-mover');
            }
            if ( ! $stylesheet_version ) {
                $errors[] = esc_html__('Error: Missing active theme version number. Please add.', 'prime-mover');
            }
        }
        $isUsingChildTheme_data_valid = false;
        if ((isset($footprint_temp['using_child_theme'])) && (!empty($footprint_temp['using_child_theme']))) {
            $isUsingChildTheme_data_valid = true;
        }
        ...
        if ((true === $template_valid) && (true === $stylesheet_valid) &&
        (true === $isUsingChildTheme_data_valid) && (true === $is_using_site_url_data_valid) && (true === $is_using_site_url_data_valid)) {
    ...
    public function sha256File($results = '')
    {
        ...
        if (is_readable($results)) {
            return hash_file('sha256', $results);
        } 
        return '';        
    }
    ...
    /**
     * Generate an SHA256 hash string from the contents of a directory or a file.
     *
     * @param string $input
     * @return boolean|string
     * @codeCoverageIgnore
     */
    public function hashEntity($input = '')
    {
        
        if (is_file($input)) {
            return $this->sha256File($input);
        } elseif (is_dir($input ) ) {
            $files = [];
            $dir = dir($input);
            while (false !== ($file = $dir->read())) {
                if ($file != '.' and $file != '..') {
                    if (is_dir($input . DIRECTORY_SEPARATOR . $file)) {
                        $files[] = $this->hashEntity($input . DIRECTORY_SEPARATOR . $file);
                    } else {
                        $files[] = $this->sha256File($input . DIRECTORY_SEPARATOR . $file);
                    }
                }
            }
            $dir->close();
     ...
     */
    public function updateBlogOption($blog_id = 0, $option = '', $value = null)
    {
        if ( ! $this->getSystemAuthorization()->isUserAuthorized()) {
            return;
        }
     ...
     * @return string
     */
    public function createFriendlyName($blogid_to_export = 0, $download_path = '', $is_wprime = false)
    {
        if (! $blogid_to_export) {
    ...
    /**
     * File put contents wrapper - append mode
     * @param string $path
     * @param string $data
     * @codeCoverageIgnore
     */
    public function filePutContentsAppend($path = '', $data = '')
    {
        if ( ! $path || ! $data ) {
            return;
        }
     ...
     */
    public function getTablesforReplacement($blog_id = 0)
    {        
        global $wpdb;
        $all_tables	= [];
        ...
        return apply_filters('prime_mover_tables_for_replacement', $all_tables, $blog_id);
    }
