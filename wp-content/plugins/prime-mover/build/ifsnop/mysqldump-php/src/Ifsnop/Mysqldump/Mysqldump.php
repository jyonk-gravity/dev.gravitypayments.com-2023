...
class Mysqldump
{
     ...
     * @return void|array
     */
    public function start($filename = '', $chunk_mode = \false, $chunk_index = 0, $batch_size = 10, $table = '', $tables = [], $table_count = 0, $primary_keys = [], $orderbykeys = [], $left_off = [])
    {
        ...
        if ($chunk_mode && $write_header) {
            ...
            $this->getDatabaseStructures(\true);
    ...
    private function getDumpFileHeader()
    {
        ...
        if (!$this->dumpSettings['skip-comments']) {
            ...
            $header = "-- mysqldump-php https://github.com/codex-m/mysqldump-php" . \PHP_EOL . "--" . \PHP_EOL . "-- Host: {$this->host}\tDatabase: {$this->dbName}" . \PHP_EOL . "-- ------------------------------------------------------" . \PHP_EOL;
    ...
    private function getDatabaseStructureTables()
    {
        ...
        if (empty($this->dumpSettings['include-tables'])) {
            ...
            foreach ($this->dbHandler->query($this->typeAdapter->show_tables($this->dbName)) as $row) {
                if (\in_array(\current($row), $this->dumpSettings['include-tables'], \true)) {
                    \array_push($this->tables, \current($row));
    ...
    private function getDatabaseStructureViews()
    {
        ...
        if (empty($this->dumpSettings['include-views'])) {
            ...
            // include only the tables mentioned in include-tables
            foreach ($this->dbHandler->query($this->typeAdapter->show_views($this->dbName)) as $row) {
                if (\in_array(\current($row), $this->dumpSettings['include-views'], \true)) {
                    \array_push($this->views, \current($row));
    ...
    private function exportTables($chunk_mode = \false, $batch_size = 10, $chunk_index = 0, $primary_keys = [], $orderbykeys = [], $left_off = [])
    {
        ...
        foreach ($this->tables as $table) {
            ...
            } elseif (\true === $this->dumpSettings['no-data'] || $this->matches($table, $this->dumpSettings['no-data'])) {
                continue;
            } else {
                $count = $this->listValues($table, $chunk_mode, $batch_size, $primary_keys, $orderbykeys, $left_off);
            }
    ...
    private function getTriggerStructure($triggerName)
    {
        ...
        foreach ($this->dbHandler->query($stmt) as $r) {
            if ($this->dumpSettings['add-drop-trigger']) {
                $this->compressManager->write($this->typeAdapter->add_drop_trigger($triggerName));
            }
    ...
    {
        if (!$this->dumpSettings['skip-comments']) {
            $ret = "--" . \PHP_EOL . "-- Dumping routines for database '" . $this->dbName . "'" . \PHP_EOL . "--" . \PHP_EOL . \PHP_EOL;
            $this->compressManager->write($ret);
        }
    ...
    {
        if (!$this->dumpSettings['skip-comments']) {
            $ret = "--" . \PHP_EOL . "-- Dumping events for database '" . $this->dbName . "'" . \PHP_EOL . "--" . \PHP_EOL . \PHP_EOL;
            $this->compressManager->write($ret);
        }
    ...
    protected function hookTransformColumnValue($tableName, $colName, $colValue, $row)
    {
        ...
        return \call_user_func_array($this->transformColumnValueCallable, array($tableName, $colName, $colValue, $row));
    }
    /**
    ...
    private function listValues($tableName, $chunk_mode = \false, $batch_size = 10, $primary_keys = [], $orderbykeys = [], $left_off = [])
    {
        ...
        foreach ($resultSet as $row) {
            ...
            if ($onlyOnce || !$this->dumpSettings['extended-insert']) {
                if ($this->dumpSettings['complete-insert']) {
                    $lineSize += $this->compressManager->write("INSERT{$ignore} INTO `{$tableName}` (" . \implode(", ", $colNames) . ") VALUES (" . \implode(",", $vals) . ")");
                } else {
    ...
    public function endListValues($tableName, $count = 0)
    {
        ...
        $this->compressManager->write(\PHP_EOL);
        if (!$this->dumpSettings['skip-comments']) {
            $this->compressManager->write("-- Dumped table `" . $tableName . "` with {$count} row(s)" . \PHP_EOL . '--' . \PHP_EOL . \PHP_EOL);
        }
        return;
    }
    /**
    ...
    public function getColumnStmt($tableName)
    {
        ...
        foreach ($this->tableColumnTypes[$tableName] as $colName => $colType) {
            if ($colType['type'] == 'bit' && $this->dumpSettings['hex-blob']) {
                ...
                $this->dumpSettings['complete-insert'] = \true;
                continue;
            } else {
                $colStmt[] = "`{$colName}`";
            }
...
{
    public static $enums = array("Sqlite", "Mysql");
    /**
     * @param string $c
     * @return boolean
     */
...
abstract class TypeAdapterFactory
{
    ...
    {
        return "SELECT tbl_name as 'View', sql as 'Create View' " . "FROM sqlite_master " . "WHERE type='view' AND tbl_name='{$viewName}'";
    }
    /**
...
class TypeAdapterMysql extends TypeAdapterFactory
{
    ...
    public $mysqlTypes = array('numerical' => array('bit', 'tinyint', 'smallint', 'mediumint', 'int', 'integer', 'bigint', 'real', 'double', 'float', 'decimal', 'numeric'), 'blob' => array(
        'tinyblob',
        'blob',
        'mediumblob',
        'longblob',
    ...
    public function databases()
    {
        ...
        $resultSet->closeCursor();
        $ret = "";
        $ret .= "CREATE DATABASE /*!32312 IF NOT EXISTS*/ `{$databaseName}`" . " /*!40100 DEFAULT CHARACTER SET {$characterSet} " . " COLLATE {$collationDb} */;" . \PHP_EOL . \PHP_EOL . "USE `{$databaseName}`;" . \PHP_EOL . \PHP_EOL;
        return $ret;
    }
    public function show_create_table($tableName)
    {
    ...
    public function create_table($row)
    {
        ...
        $ret = "/*!40101 SET @saved_cs_client     = @@character_set_client */;" . \PHP_EOL . "/*!40101 SET character_set_client = " . $this->dumpSettings['default-character-set'] . " */;" . \PHP_EOL . $createTable . ";" . \PHP_EOL . "/*!40101 SET character_set_client = @saved_cs_client */;" . \PHP_EOL . \PHP_EOL;
        return $ret;
    }
    public function create_view($row)
    {
        ...
        if ($viewStmtReplaced = \preg_replace('/^(CREATE(?:\\s+ALGORITHM=(?:UNDEFINED|MERGE|TEMPTABLE))?)\\s+(' . self::DEFINER_RE . '(?:\\s+SQL SECURITY DEFINER|INVOKER)?)?\\s+(VIEW .+)$/', '/*!50001 \\1 */' . \PHP_EOL . $definerStr . '/*!50001 \\3 */', $viewStmt, 1)) {
    ...
    public function create_trigger($row)
    {
        ...
        if ($triggerStmtReplaced = \preg_replace('/^(CREATE)\\s+(' . self::DEFINER_RE . ')?\\s+(TRIGGER\\s.*)$/s', '/*!50003 \\1*/ ' . $definerStr . '/*!50003 \\3 */', $triggerStmt, 1)) {
            $triggerStmt = $triggerStmtReplaced;
        }
        $ret .= "DELIMITER ;;" . \PHP_EOL . $triggerStmt . ";;" . \PHP_EOL . "DELIMITER ;" . \PHP_EOL . \PHP_EOL;
        return $ret;
    }
    public function create_procedure($row)
    {
        ...
        if ($this->dumpSettings['skip-definer']) {
            if ($procedureStmtReplaced = \preg_replace('/^(CREATE)\\s+(' . self::DEFINER_RE . ')?\\s+(PROCEDURE\\s.*)$/s', '\\1 \\3', $procedureStmt, 1)) {
    ...
    public function create_function($row)
    {
        ...
        if ($this->dumpSettings['skip-definer']) {
            if ($functionStmtReplaced = \preg_replace('/^(CREATE)\\s+(' . self::DEFINER_RE . ')?\\s+(FUNCTION\\s.*)$/s', '\\1 \\3', $functionStmt, 1)) {
                $functionStmt = $functionStmtReplaced;
            }
        }
        $ret .= "/*!50003 DROP FUNCTION IF EXISTS `" . $row['Function'] . "` */;" . \PHP_EOL . "/*!40101 SET @saved_cs_client     = @@character_set_client */;" . \PHP_EOL . "/*!40101 SET character_set_client = " . $this->dumpSettings['default-character-set'] . " */;" . \PHP_EOL . "DELIMITER ;;" . \PHP_EOL . $functionStmt . " ;;" . \PHP_EOL . "DELIMITER ;" . \PHP_EOL . "/*!40101 SET character_set_client = @saved_cs_client */;" . \PHP_EOL . \PHP_EOL;
        return $ret;
    }
    public function create_event($row)
    {
        ...
        if ($eventStmtReplaced = \preg_replace('/^(CREATE)\\s+(' . self::DEFINER_RE . ')?\\s+(EVENT .*)$/', '/*!50106 \\1*/ ' . $definerStr . '/*!50106 \\3 */', $eventStmt, 1)) {
            $eventStmt = $eventStmtReplaced;
        }
        $ret .= "/*!50106 SET @save_time_zone= @@TIME_ZONE */ ;" . \PHP_EOL . "/*!50106 DROP EVENT IF EXISTS `" . $eventName . "` */;" . \PHP_EOL . "DELIMITER ;;" . \PHP_EOL . "/*!50003 SET @saved_cs_client      = @@character_set_client */ ;;" . \PHP_EOL . "/*!50003 SET @saved_cs_results     = @@character_set_results */ ;;" . \PHP_EOL . "/*!50003 SET @saved_col_connection = @@collation_connection */ ;;" . \PHP_EOL . "/*!50003 SET character_set_client  = utf8 */ ;;" . \PHP_EOL . "/*!50003 SET character_set_results = utf8 */ ;;" . \PHP_EOL . "/*!50003 SET collation_connection  = utf8_general_ci */ ;;" . \PHP_EOL . "/*!50003 SET @saved_sql_mode       = @@sql_mode */ ;;" . \PHP_EOL . "/*!50003 SET sql_mode              = '" . $sqlMode . "' */ ;;" . \PHP_EOL . "/*!50003 SET @saved_time_zone      = @@time_zone */ ;;" . \PHP_EOL . "/*!50003 SET time_zone             = 'SYSTEM' */ ;;" . \PHP_EOL . $eventStmt . " ;;" . \PHP_EOL . "/*!50003 SET time_zone             = @saved_time_zone */ ;;" . \PHP_EOL . "/*!50003 SET sql_mode              = @saved_sql_mode */ ;;" . \PHP_EOL . "/*!50003 SET character_set_client  = @saved_cs_client */ ;;" . \PHP_EOL . "/*!50003 SET character_set_results = @saved_cs_results */ ;;" . \PHP_EOL . "/*!50003 SET collation_connection  = @saved_col_connection */ ;;" . \PHP_EOL . "DELIMITER ;" . \PHP_EOL . "/*!50106 SET TIME_ZONE= @save_time_zone */ ;" . \PHP_EOL . \PHP_EOL;
    ...
    public function show_procedures()
    {
        $this->check_parameters(\func_num_args(), $expected_num_args = 1, __METHOD__);
        $args = \func_get_args();
        return "SELECT SPECIFIC_NAME AS procedure_name " . "FROM INFORMATION_SCHEMA.ROUTINES " . "WHERE ROUTINE_TYPE='PROCEDURE' AND ROUTINE_SCHEMA='{$args[0]}'";
    }
    public function show_functions()
    {
        $this->check_parameters(\func_num_args(), $expected_num_args = 1, __METHOD__);
        $args = \func_get_args();
        return "SELECT SPECIFIC_NAME AS function_name " . "FROM INFORMATION_SCHEMA.ROUTINES " . "WHERE ROUTINE_TYPE='FUNCTION' AND ROUTINE_SCHEMA='{$args[0]}'";
    }
    /**
    ...
    public function show_events()
    {
        $this->check_parameters(\func_num_args(), $expected_num_args = 1, __METHOD__);
        $args = \func_get_args();
        return "SELECT EVENT_NAME AS event_name " . "FROM INFORMATION_SCHEMA.EVENTS " . "WHERE EVENT_SCHEMA='{$args[0]}'";
    }
    public function setup_transaction()
    {
    ...
    public function getDatabaseHeader()
    {
        $this->check_parameters(\func_num_args(), $expected_num_args = 1, __METHOD__);
        $args = \func_get_args();
        return "--" . \PHP_EOL . "-- Current Database: `{$args[0]}`" . \PHP_EOL . "--" . \PHP_EOL . \PHP_EOL;
    }
    /**
     * Decode column metadata and fill info structure.
     * type, is_numeric and is_blob will always be available.
    ...
    public function backup_parameters()
    {
        $ret = "/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;" . \PHP_EOL . "/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;" . \PHP_EOL . "/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;" . \PHP_EOL . "/*!40101 SET NAMES " . $this->dumpSettings['default-character-set'] . " */;" . \PHP_EOL;
        if (\false === $this->dumpSettings['skip-tz-utc']) {
            $ret .= "/*!40103 SET @OLD_TIME_ZONE=@@TIME_ZONE */;" . \PHP_EOL . "/*!40103 SET TIME_ZONE='+00:00' */;" . \PHP_EOL;
        }
        $ret .= "/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0 */;" . \PHP_EOL . "/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;" . \PHP_EOL . "/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;" . \PHP_EOL . "/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;" . \PHP_EOL . \PHP_EOL;
        return $ret;
    }
    public function restore_parameters()
    {
        ...
        $ret .= "/*!40101 SET SQL_MODE=@OLD_SQL_MODE */;" . \PHP_EOL . "/*!40014 SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS */;" . \PHP_EOL . "/*!40014 SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS */;" . \PHP_EOL . "/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;" . \PHP_EOL . "/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;" . \PHP_EOL . "/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;" . \PHP_EOL . "/*!40111 SET SQL_NOTES=@OLD_SQL_NOTES */;" . \PHP_EOL . \PHP_EOL;
        return $ret;
    }
    /**
