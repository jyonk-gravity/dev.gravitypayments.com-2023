				...
				$.ajax({
					...
					success: function( response ) {
						if (PrimeMoverCoreJsObject.prime_mover_is_js_object(response) && 'retry_download' in response) {
							...
							setTimeout(function() { PrimeMoverCoreJsObject.fetchFileToRestore(package_url, blog_id); }, 2000); 
						}
					},
				...
				$.ajax({
					...
					success: function( response ) {
						...
						if (PrimeMoverCoreJsObject.prime_mover_is_js_object(response)) {							
							...
							if (data.noresponsecount <= this.retryLimit) {                                
								var pulse = true;
								PrimeMoverCoreJsObject[refreshid] = setTimeout(function() { PrimeMoverCoreJsObject.progressAjax(data, mode, blog_id, progress_identifier, refreshid); }, 7000);	 	                         
					...
					error : function(xhr, textStatus, errorThrown ) {
						...
						if (data.errorcount <= this.retryLimit) {
							...
							PrimeMoverCoreJsObject.logProcessingErrorAnalysis(blog_id, args, mode, 'progressAjax');
							PrimeMoverCoreJsObject[refreshid] = setTimeout(function() { PrimeMoverCoreJsObject.progressAjax(data, mode, blog_id, progress_identifier, refreshid, args); }, 7000);   	                         
			...
			retryAjaxHandler: function(ajax_handler_object, mode, blog_id, source) {
				...
				if ('import' === mode) {
					if (ajax_handler_object.tryCount <= ajax_handler_object.retryLimit) {
						...
						setTimeout(function() { PrimeMoverCoreJsObject.retryHelper(ajax_handler_object); }, 7000);            		
						return;
			...
			doExporterAjax: function(blog_id, data, retry_times, export_processing_times) {	
				...
				$.post(ajaxurl, data, function( response ) {	
					if (PrimeMoverCoreJsObject.prime_mover_is_js_object(response) && 'next_method' in response && 'current_method' in response) {
						...
						setTimeout(function() { PrimeMoverCoreJsObject.doExporterAjax(blog_id, data); }, 1000);	 	    		
					}	 	    	
				}).fail(function(xhr, status, error) {
					...
					if (total_time_spent_retrying > retryLimit) {   
						...
						setTimeout(function() { PrimeMoverCoreJsObject.doExporterAjax(blog_id, data, retry_times, export_processing_times); }, 7000);	  						
					} 	 	    
				});    		
			},			
			/**
				...
				$.ajax({
					...
					success: function( response ) {
						...
						if (PrimeMoverCoreJsObject.prime_mover_is_js_object(response) && 'next_method' in response && 'current_method' in response) {
							...
							setTimeout(function() { PrimeMoverCoreJsObject.import_handler(blog_id, filepath, retry_times, ajaxData); }, 1000);	 	    		
						} 	     
					},
					error : function(xhr, textStatus, errorThrown ) {	
						var mode = 'import';
			...
			doImportHandlingRetry: function(ajaxData, errorThrown, blog_id, retry_times, filepath, diffmode, start_time, import_processing_times) {				
				...
				if (total_time_spent_retrying > retryLimit) {			
					...
					if (diffmode) {
						...
						setTimeout(function() { PrimeMoverCoreJsObject.doDiffPostAjax(mode, blog_id, ajaxData, retry_times, import_processing_times); }, 7000);					
					} else {
						...
						setTimeout(function() { PrimeMoverCoreJsObject.import_handler(blog_id, filepath, retry_times, ajaxData, import_processing_times); }, 7000);						
					}					
				}							
			},
			/**
				...
				$.ajax({
					...
					success: function( response ) {	
						...
						if (PrimeMoverCoreJsObject.prime_mover_is_js_object(response) && 'next_method' in response && 'current_method' in response) {
							...
							setTimeout(function() { PrimeMoverCoreJsObject.doDiffPostAjax(mode, blog_id, args); }, 1000);	 	    		
						}						
					},
					error : function(xhr, textStatus, errorThrown ) {
