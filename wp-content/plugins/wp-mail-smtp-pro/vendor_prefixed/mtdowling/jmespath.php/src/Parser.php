...
class Parser
{
    ...
    {
        static $validKeys = [T::T_QUOTED_IDENTIFIER => \true, T::T_IDENTIFIER => \true];
        $this->next($validKeys);
        $pairs = [];
        do {
    ...
    {
        static $nextTypes = [T::T_NUMBER => \true, T::T_COLON => \true, T::T_STAR => \true];
        $this->next($nextTypes);
    ...
    {
        $this->next();
        return ['type' => 'projection', 'from' => 'array', 'children' => [['type' => T::T_FLATTEN, 'children' => [$left]], $this->parseProjection(self::$bp[T::T_FLATTEN])]];
    }
    private function led_dot(array $left)
    {
        ...
        return ['type' => 'subexpression', 'children' => [$left, $this->parseDot(self::$bp[T::T_DOT])]];
    }
    private function led_or(array $left)
    {
        $this->next();
        return ['type' => T::T_OR, 'children' => [$left, $this->expr(self::$bp[T::T_OR])]];
    }
    private function led_and(array $left)
    {
        $this->next();
        return ['type' => T::T_AND, 'children' => [$left, $this->expr(self::$bp[T::T_AND])]];
    }
    private function led_pipe(array $left)
    {
        $this->next();
        return ['type' => T::T_PIPE, 'children' => [$left, $this->expr(self::$bp[T::T_PIPE])]];
    }
    ...
    private function led_comparator(array $left)
    {
        $token = $this->token;
        $this->next();
        return ['type' => T::T_COMPARATOR, 'value' => $token['value'], 'children' => [$left, $this->expr(self::$bp[T::T_COMPARATOR])]];
    }
    ...
    {
        static $validColon = [T::T_COLON => \true];
    ...
    private function parseWildcardArray(?array $left = null)
    {
        ...
        $this->next();
        return ['type' => 'projection', 'from' => 'array', 'children' => [$left ?: self::$currentNode, $this->parseProjection(self::$bp[T::T_STAR])]];
    }
    /**
    ...
    {
        static $matchNext = [T::T_NUMBER => \true, T::T_COLON => \true, T::T_RBRACKET => \true];
        $pos = 0;
        ...
        do {
            if ($this->token['type'] == T::T_COLON) {
                ...
                $expected = [T::T_COLON => \true, T::T_RBRACKET => \true];
            }
            $this->next($expected);
        ...
        return ['type' => 'projection', 'from' => 'array', 'children' => [['type' => 'slice', 'value' => $parts], $this->parseProjection(self::$bp[T::T_STAR])]];
    }
